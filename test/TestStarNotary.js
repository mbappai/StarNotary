...
it('has correct name', async () => {
    ...
    assert.equal(starName, "My awesome star"); // Assert if the starName property was initialized correctly
});
...
it('can change owners', async () => {
 ...
 it('can change name', async ()=>{
     let instance = await StarNotary.deployed();
     let newStarName = "Mujeex-star";
     try{
         await instance.claimStar({from:owner})
         await instance.changeStarName(newStarName,{from:owner})
     }finally{
         let updatedStarName = await instance.starName.call();
